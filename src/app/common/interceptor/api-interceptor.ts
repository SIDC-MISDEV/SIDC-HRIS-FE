...
export class ApiInterceptor  implements HttpInterceptor {
  ...
  intercept(request: HttpRequest<any>, next: HttpHandler): Observable<HttpEvent<any>> {
    ...
    if (request.url.includes("psgc.gitlab.io") 
    || request.url.includes("login") 
    || request.url.includes("signup") 
    || request.url.includes("forgot-password") 
    || request.url.includes("/identity/reset-password")
    || request.url.includes("/b1s/v1/Login"))
      return next.handle(request);

    // const token: string = localStorage.getItem('token');
    let token: string = localStorage.getItem('token');
    let SapToken: string | null = sessionStorage.getItem('SapToken') ?? null;

    if(request.url.includes("/b1s/v1/"))
    {
        request = request.clone({ headers: request.headers.set('Authorization', SapToken) });
